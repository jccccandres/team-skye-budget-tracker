...
import { GroceryInviteBanner } from '../components/grocery/GroceryInviteBanner'
import { GroceryListForm } from '../components/grocery/GroceryListForm'
import { GroceryShareForm } from '../components/grocery/GroceryShareForm'
...
export function GroceryListsPage() {
  ...
  const [sharing, setSharing] = useState<GroceryList | null>(null)
  ...
  return (
    <div>
      ...
      {sharing && (
        <Modal title="Share grocery list" onClose={() => setSharing(null)}>
          <GroceryShareForm
            listName={sharing.name}
            onSubmit={(email) => inviteToList(sharing.id, email)}
