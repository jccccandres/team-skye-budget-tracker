...
CREATE POLICY "Members can view their wallets"
  ...
  USING (id IN (SELECT wallet_id FROM wallet_members WHERE user_id = auth.uid()));
...
CREATE POLICY "Members can view wallet membership"
  ...
  USING (wallet_id IN (SELECT wallet_id FROM wallet_members WHERE user_id = auth.uid()));

CREATE POLICY "Owner can add the creator as first member"
  ON wallet_members FOR INSERT
  WITH CHECK (
    user_id = auth.uid()
    OR wallet_id IN (SELECT id FROM wallets WHERE created_by = auth.uid())
  );
...
CREATE POLICY "Wallet members can create invites"
  ...
  WITH CHECK (wallet_id IN (SELECT wallet_id FROM wallet_members WHERE user_id = auth.uid()));
...
CREATE POLICY "Users can view own or wallet expenses"
  ...
  USING (
    ...
    OR (wallet_id IN (SELECT wallet_id FROM wallet_members WHERE user_id = auth.uid()))
  );
